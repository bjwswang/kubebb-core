...
func (r *PortalReconciler) GetDuplicated(ctx context.Context, o client.Object) (reqs []reconcile.Request, err error) {
	portal := o.(*corev1alpha1.Portal)

	// select with entry labels
	var portalList = corev1alpha1.PortalList{}
	err = r.Client.List(ctx, &portalList)
	if err != nil {
		return
	}
	...
	for _, p := range portalList.Items {
		...
		if _, ok := dupPortalMap[p.Name]; !ok {
			dupPortalMap[p.Name] = true
			reqs = append(reqs, reconcile.Request{
				NamespacedName: types.NamespacedName{
					Name: p.Name,
				},
