...
function checkPortalStatus() {
  ...
  while true; do
    ...
    if [[ $expectConflictsInEntry == $conflictsInEntry ]] && [[ $expectConflictsInPath == $conflictsInPath ]]; then
      echo "Portal ${portalName} has the correct status in conflicted entry and path"
      break
    fi

    echo "expectConflictsInEntry:$expectConflictsInEntry  Actual:$conflictsInEntry"
    echo "expectConflictsInPath:$expectConflictsInPath  Actual:$conflictsInPath"
...
info "8.3 create a dupolicate portal which both conflicts in entry and path"
